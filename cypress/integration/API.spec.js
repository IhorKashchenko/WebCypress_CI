...
const allUrl = [
  ...
  baseUrlQA +
    "/DocLinkWebService/api/Workflow/QueuesForWorkflow?workflowId=10000",
  ...
  baseUrlQA +
    "/DocLinkWebService/api/Document/GetOriginationData?documentId=17",
  baseUrlQA + "/DocLinkWebService/api/Document/GetIndexData?documentId=17",
  baseUrlQA + "/DocLinkWebService/api/Document/Notes?id=17",
  baseUrlQA +
    "/DocLinkWebService/api/Document/GetAnnotationSummary?documentId=17",
  baseUrlQA + "/DocLinkWebService/api/Document/GetAuditHist?documentId=17",
  baseUrlQA + "/DocLinkWebService/api/Document/GetIndexData?documentId=17",
  baseUrlQA +
    "/DocLinkWebService/api/DocumentImage/GetPageProperties?documentId=17&pageNum=1",
...
const start = 45337;
const end = 45999;
const docId = [...Array(end - start + 1).keys()].map((x) => x + start);
const docIdBad = [];
...
describe("API test", () => {
  before("GET AuthCode", () => {
    cy.request({
      ...
      body: {
        ...
        ProfileData:
          "Kz6Bbq1Loa6mleIvq1gUxf2uj5jTd6c1nkb7Ie5TGk9+bPvBl//c9NksQMRVxyN/",
        ...
        WindowsAuth: false,
        Password:
          "CdoQpM2hdFYtBysB54x0r+1+u1rqCGwS3Zu/uILoaLihHdiwBn0+1x05hvhdpr4u1RCGA0bG9anWb+t11au6ODrweYaDo2c8zlZixqyL7e1EvEDQqpX9bMGueV8rfV2L6GRPgyXPw4G/qEFeXXQq7ba9Gv8uX3z70kztq3U4l6o=",
        RememberMe: true,
        ImpersonateForWindowsAuthentication: false,
  ...
  it("Get Doc ID data for doc 123", () => {
    cy.request({
      ...
      url:
        baseUrlQA +
        "/DocLinkWebService/api/Document/GetOriginationData?documentId=123",
      headers: {
        AuthCode: window.token,
      },
    }).then((res) => {
  ...
  it("search for missing ORIGIN module", () => {
    for (let i = 0; i < docId.length; i++) {
      cy.request({
        method: "GET",
        url:
          "https://wa19qa40.altec-wa.com/DocLinkWebService/api/DocumentImage/GetPageProperties?documentId=" +
          docId[i] +
          "&pageNum=1",
        headers: {
          AuthCode: window.token,
        ...
        if (
          (res.status == 500) &
          (res.body.ExceptionType == "System.IndexOutOfRangeException")
        ) {
          docIdBad.push(docId[i]);
        }
      });
    }
    cy.log(JSON.stringify(docIdBad)).then(() => {
        if(docIdBad.length > 0) {
            throw new Error('Missing Origination');
        }
    })
  });
});
